...
public class GestorePrenotazioni implements Serializable {
    ...
    //HashMap creato per avere un dizionario composto dalle date (come key) e dalle prenotazioni ( come valori)
    private HashMap<Date, Affitto> calendario = new HashMap<Date, Affitto>();
    ...
    public void aggiungiPrenotazione() {
        try {
            ...
            if (!calendario.containsKey(d)) {
                ...
                //variabile prenotazione di tipo affitto che conterrà le possibili future prenotazioni
                ...
                if (!registro.containsKey(nome)) {
                    ...
                    //se la persona è già presente, prendere la lista di prenotazioni a lei associata dal registro
                    ...
                    // reinserire le prenotazioni modificate nel registro
                    registro.replace(nome, prenotazioniEsistenti);
                }
                // associo la prenotazione alla data
                calendario.put(d, prenotazione);
            } else
    ...
    public void visualizzaPrenotazioni() {
        ...
        if (!dateOrdinate.isEmpty()) {
            ...
            for (Date d : dateOrdinate) {
                System.out.println(calendario.get(d));
            }
        } else
    ...
    // l'argomento "stampaAncheCatering" agisce come flag per indicare se stampare anche le prenotazioni di
    // tipo Catering oltre che quelle di tipo CateringAnimazione
    public void stampaCatering(boolean stampaAncheCatering) {
    	boolean ci_sono_prenotazioni = false;
        ...
        // se non ci sono prenotazioni, stampiamo un messaggio per avvisare
        if (!ci_sono_prenotazioni) {
            System.out.println("Non ci sono ancora prenotazioni di questo tipo");
        }
    }
        ...
        do {
            ...
            try {
                ...
                if (calendario.containsKey(dataEliminata)) {
                    ...
                    String nome = calendario.get(dataEliminata).getCliente();
    ...
    // metodo per cercare le prenotazioni partendo dal nome del cliente o da una sua porzione
    // restituisce il vettore di prenotazioni associate al cliente oppure null se non esiste
    ...
    public void importaPrenotazioni() {
        ...
        System.out.println("Prenotazioni importate con successo!");
    }
}
